      ...
      <ul className='text-left overflow-auto mx-5 mt-2 '>
        {
          recentHistory && recentHistory.map((item, index)=> (
            <li key={index} onClick={()=> setSelectedHistory(item)} className='p-1 pl-5 truncate px-5 dark:text-zinc-300 text-zinc-700 cursor-pointer dark:hover:bg-zinc-700 hover:bg-red-200 hover:text-zinc-600  dark:hover:text-zinc-100'>{item}</li>
          ))
        }
      </ul>
      </div>
